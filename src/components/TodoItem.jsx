...
function TodoItem(props){
    ...
    return (
        <div className={`relative w-72 h-auto min-h-6 p-2 text-mdrounded-xl font-semibold border-4 mb-2 rounded-xl border-zinc-600 ${styleCompleted}`}>
            ...
            <p>{props.text}</p>
            <span className={`absolute -right-2 -top-2 rounded-full cursor-pointer hover:text-indigo-100 hover:bg-red-900 ${styleClickDeleteIcon}`} onClick={props.onDelete}><DeleteIcon /></span>
        </div>
    )
}
